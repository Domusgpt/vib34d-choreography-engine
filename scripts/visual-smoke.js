...
async function run() {
    ...
    try {
        ...
        if (await visualButton.count()) {
            ...
            await visualButton.click({ delay: 50, timeout: 3000 }).catch(err => console.warn('Visual test trigger skipped:', err.message));
        ...
        if (await stackButton.count()) {
            ...
            await stackButton.click({ timeout: 1500 }).catch(err => console.warn('Stack log trigger skipped:', err.message));
        }
