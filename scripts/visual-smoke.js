...
async function forceVisibleClick(page, locator, label) {
    ...
    try {
        ...
        // Try to bring the control into view from both ends of the page
        await page.evaluate(() => window.scrollTo({ top: 0, behavior: 'instant' }));
        await page.$eval('#drawer-content', el => el.scrollTo({ top: el.scrollHeight, behavior: 'instant' })).catch(() => {});
        ...
        await target.click({ delay: 50, timeout: 8000, force: true });
        return true;
    } catch (err) {
        try {
            ...
            console.warn(`${label} force click via evaluate failed:`, evalErr.message || evalErr);
        }
