...
const ignoreConsolePatterns = [
  { pattern: /GroupMarkerNotSet/, note: 'headless marker noise' },
  { pattern: /GL Driver Message .*GPU stall due to ReadPixels/, note: 'headless GPU stall noise' },
  { pattern: /Automatic fallback to software WebGL/, note: 'software WebGL fallback noise' }, // expected in CI/headless
];

function findIgnoredConsole(entry) {
  return ignoreConsolePatterns.find(({ pattern }) => pattern.test(entry));
}
  ...
  try {
    ...
    page.on('console', (msg) => {
      ...
      if (msg.type() === 'error' || msg.type() === 'warning') {
        const ignored = findIgnoredConsole(entry);
        if (ignored) {
          ignoredConsole.push({ entry, note: ignored.note });
        } else {
    ...
    if (ignoredConsole.length) {
      console.log(
        `Ignored ${ignoredConsole.length} console warnings/errors during snapshot (headless WebGL noise): ${[...new Set(
          ignoredConsole.map(({ note }) => note),
        )].join(', ')}`,
      );
    }

    if (consoleErrors.length) {
      console.warn(`Captured ${consoleErrors.length} actionable console warnings/errors during snapshot.`);
    }
