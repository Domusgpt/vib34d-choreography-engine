...
## Latest Update: Renderer Reliability + Sweep Director

### 🛡️ Auto-healing renderer swaps
- Added a renderer health check that watches for lost WebGL contexts and automatically rebuilds the active visualizer with the latest parameters instead of leaving the canvas blank.
- Centralised visualizer binding so swaps, retries, and health-driven restarts all hydrate parameters, reattach the shader choreographer, and update the status HUD consistently.

## Sweep Director + Behavioral Console Refactor
