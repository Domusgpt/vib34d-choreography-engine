...
### 🗂️ Queue control, weighting, and history
- Scene Director now supports weighted shuffle pools, global minimum durations, and one-off queues—perfect for dropping a special scene on the next downbeat without changing playlists.
- Both consoles surface queue management (play next, remove, clear) and a timestamped history ledger so you can replay crowd-favourite looks with a single tap.

### 🧰 Scene cards gain quick actions
- Ultimate Reactive cards ship with inline queue buttons and weight sliders so shuffle probability is adjustable without leaving the grid.
- Mobile Maestro mirrors the queue, weighting, and history interface in the thumb-friendly deck so phones get the same live control as desktops.
