...
The prompt template lives in `LLM_BEHAVIOR_PROMPT_TEMPLATE` and asks the LLM to respond using simple colon-delimited text (no JSON). Supported keys: `Preset`, `Intensity`, `Density`, `Morph`, `Color`, `Chaos`, `Rotation`, `Camera`, **`Atmosphere`**, **`Light`**, and an optional `Notes` line for human operators.
