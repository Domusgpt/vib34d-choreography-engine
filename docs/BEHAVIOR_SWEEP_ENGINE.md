...
- `atmosphere`: `{ parallaxDepth, gridShift, shimmer }` volumetric modifiers for depth parallax, lattice shifting, and shimmer/halo accents.
...
The prompt template lives in `LLM_BEHAVIOR_PROMPT_TEMPLATE` and asks the LLM to respond using simple colon-delimited text (no JSON). Supported keys: `Preset`, `Intensity`, `Density`, `Morph`, `Color`, `Chaos`, `Rotation`, `Camera`, **`Atmosphere`**, and an optional `Notes` line for human operators.
