...
- `color`: `{ hueShift, saturationPulse, spark }` colour modulation suitable for hue/saturation plus a sparkle pulse you can map to highlights.
- `light`: `{ glow, bloom, strobe }` luminance helpers for volumetric bloom overlays or UI glow meters.
- `atmosphere`: `{ parallaxDepth, gridShift, shimmer, warp, haze }` volumetric modifiers for depth parallax, lattice shifting, shimmer/halo accents, warp curvature, and haze density.
- `fx`: `{ trail, chroma, flare }` cinematic overlays for trail persistence, chromatic fringing, and flare pops to layer on top of light/atmosphere.
- `tone`: `{ bass, mid, air }` spectral emphasis that tracks sub/mid/high energy so UIs or systems can bias hue/atmosphere without relying on JSON config.
...
const reactiveParams = applyReactiveControls({
  ...
  controls: {
    ...
    atmosphere: { enabled: reactivity.enabled.atmosphere },
    light: { enabled: reactivity.enabled.light },
    fx: { enabled: reactivity.enabled.fx },
  },
  limits: {
    ...
    chaos: { min: 0, max: 1 },
    speed: { min: 0, max: 3 },
    parallaxDepth: { min: -1.5, max: 1.5 },
    gridDensityShift: { min: -2.5, max: 2.5 },
    shimmer: { min: 0, max: 2 },
    glow: { min: 0, max: 2 },
    bloom: { min: 0, max: 2 },
    trailPersistence: { min: 0, max: 2 },
    chromaFringe: { min: 0, max: 2 },
    flare: { min: 0, max: 2.5 },
  }
});
```
...
Any new UI surface should follow the same pattern: keep a base parameter set, fetch deltas every frame, and apply only the deltas that correspond to enabled controls (including the optional `atmosphere` gate for parallax/shimmer heavy scenes).
...
The prompt template lives in `LLM_BEHAVIOR_PROMPT_TEMPLATE` and asks the LLM to respond using simple colon-delimited text (no JSON). Supported keys: `Preset`, `Intensity`, `Density`, `Morph`, `Color`, `Chaos`, `Rotation`, `Camera`, **`Atmosphere`**, **`Light`**, **`FX`**, **`Tone`**, and an optional `Notes` line for human operators.
...
3. Applies the parsed control gates (including the optional `atmosphere` toggle for parallax/shimmer) without requiring JSON.
