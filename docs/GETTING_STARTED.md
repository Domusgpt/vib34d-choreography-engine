...
Both flagship consoles (`ultimate-reactive.html` on desktop and `mobile-smart.html` on phones) now surface a **Hypercolor ↔ Uniform** toggle that applies to Quantum, Holographic, and Faceted visualizers alike. A few tips when testing the new colour flow:
