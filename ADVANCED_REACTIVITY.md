...
#### Quantum Palette Modes & Moiré Glitch Reactor

- **Palette library** – Switch between Legacy Quantum, Hypercolor Orbit, Aurora Bloom, Solar Inferno, and Prismatic Pop. Each palette defines per-layer primary/secondary/accent hues so background, shadow, content, highlight, and accent layers can react with unique harmonies.
- **Dynamic blending** – Hypercolor Orbit keeps sampling the Hypercolor engine every frame, while the cinematic palettes blend static base tones with live colour choreography for smooth but punchy transitions.
- **RGB moiré reactor** – A new audio-reactive toggle injects moiré lattices, RGB channel offsets, and chromatic glitch streaks. Base intensity and separation sliders control how wild the effect becomes so performers can keep it subtle or unleash full-spectrum chaos.
