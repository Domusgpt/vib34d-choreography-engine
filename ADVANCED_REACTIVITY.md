...
## 🧭 Unified Ultimate Console

The desktop **Ultimate Reactive** and mobile **Mobile Maestro** surfaces now share a single responsive console so every operator launches the same workflow regardless of device.

- **Seven-Band Transport HUD** – Live sub/bass/low/mid/high-mid/high/air meters, timeline progress, beat counters, and track badges make it obvious when real audio is feeding the choreography engine.
- **Reactivity Deck** – A master reactivity slider plus density/morph/chaos/rotation/colour/vibrance/glitch toggles lets you blend or disable channels without losing the slider baselines.
- **Palette Quick Picks** – Hypercolor ↔ Uniform style pills, curated playlist chips, and manual palette buttons live in both layouts so colour journeys stay synchronized.
- **Module Embed** – The console now lives in `examples/js/ultimateConsole.js`; call `launchUltimateConsole({ hostId, canvasId })` from any HTML shell to mount the full UI.
- **Mobile Launch** – `examples/mobile-smart.html` loads the same console module but boots straight into the bottom-sheet deck for phones and tablets.

Use the `?layout=mobile` query string (e.g., [`ultimate-reactive.html?layout=mobile`](examples/ultimate-reactive.html?layout=mobile)) to force the bottom-sheet layout when testing on desktop browsers.
...
- **Hypercolor vs. Uniform styles** – Quantum, Holographic, and Faceted visualizers now expose a shared `colorStyle` toggle so you can switch between multi-spectrum hypercolor journeys and uniform palette families without losing choreography metadata.
