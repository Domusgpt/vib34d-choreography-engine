...
let lastRendererHealthCheck = performance.now();

const ensureRendererHealth = (timestamp = performance.now()) => {
    if (timestamp - lastRendererHealthCheck < 1200) return;
    lastRendererHealthCheck = timestamp;

    const gl = currentVisualizer?.gl;
    const contextLost = typeof gl?.isContextLost === 'function' && gl.isContextLost();
    if (!currentVisualizer || !gl || contextLost) {
        setRendererNotice({ state: 'Recovering…', detail: 'Restarting renderer', error: true });
        swapVisualizer(currentSystem);
    }
};
...
const bindVisualizer = (visualizer, systemLabel = currentSystem) => {
    ...
    if (visualizerStatus && systemLabel) {
        visualizerStatus.textContent = systemLabel.charAt(0).toUpperCase() + systemLabel.slice(1);
    }
};
...
const swapVisualizer = (system) => {
    ...
    if (!nextVisualizer) {
        ...
        bindVisualizer(buildVisualizer('quantum'), 'quantum');
        return;
    }

    currentSystem = system;
    bindVisualizer(nextVisualizer, system);
};
...
if (rendererRetry) {
    rendererRetry.addEventListener('click', () => {
        ...
        if (restarted) {
            releaseVisualizer(currentVisualizer);
            bindVisualizer(restarted, currentSystem);
        }
    });
}
