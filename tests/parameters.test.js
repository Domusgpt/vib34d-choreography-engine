...
describe('ParameterManager', () => {
  ...
  it('normalizes driver weights when zero or negative values are provided', () => {
    const pm = new ParameterManager();
    pm.setDriverWeights({ beat: -1, onset: 0, audio: 0 });

    expect(pm.driverWeights.beat).toBeCloseTo(0.5, 5);
    expect(pm.driverWeights.onset).toBeCloseTo(0.5, 5);
    expect(pm.driverWeights.audio).toBeCloseTo(0, 5);
  });

  it('honors the driver floor by muting reactive offsets under the threshold', () => {
    const pm = new ParameterManager({ driverFloor: 0.25 });
    pm.registerParameters({ contrast: { baseline: 0.8 } });
    pm.setProfiles({ contrast: { range: 1, limit: 0.5 } });

    const resolved = pm.resolve({ beatEnvelope: 0.1, onsetEnvelope: 0, audioLevel: 0 });

    expect(resolved.contrast).toBeCloseTo(0.8, 5);

    const diagnostics = pm.getDiagnostics().parameters.contrast;
    expect(diagnostics.flags.hitFloor).toBe(true);
    expect(diagnostics.driver).toBe(0);
    expect(diagnostics.offset).toBe(0);
  });
});
