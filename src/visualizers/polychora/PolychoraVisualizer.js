...
void main() {
    ...
    vec3 baseColor = mix(
        u_colorPrimary,
        u_colorSecondary,
        clamp(layerMix + orbit * 0.4 + beat * 0.2 + pointerMag * 0.3, 0.0, 1.0)
    );
    vec3 accentColor = mix(baseColor, u_colorAccent, clamp(accent * 0.7 + onset * 0.6 + downbeat * 0.5, 0.0, 1.0));

    vec3 finalColor = mix(baseColor, accentColor, grid);
    finalColor += glow * (0.3 + accentLuma * 0.4 + pointerMag * 0.2);

    float vignette = 0.82 + 0.18 * cos(length(uv) * 3.2 - time * 0.3 + ribbon * 2.0);
    finalColor *= vignette;
