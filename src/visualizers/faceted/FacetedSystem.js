...
export class FacetedSystem extends BaseSystem {
    ...
    setParameterTuning(tuning = {}) {
        this.parameterTuning = {
            ...
            driverWeights: {
                ...this.parameterTuning.driverWeights,
                ...(tuning.driverWeights || {})
            },
            driverFloor: tuning.driverFloor ?? this.parameterTuning.driverFloor
        };
        ...
        if (this.parameters) {
            ...
            this.parameters.setDriverWeights(this.parameterTuning.driverWeights);
            this.parameters.setDriverFloor(this.parameterTuning.driverFloor);
        }
    }
    ...
    async createVisualizer() {
        ...
        this.parameters.registerParameters({
            volumetricDensity: { baseline: 0.28, profile: { range: 0.32, min: 0, max: 1, limit: this.parameterTuning.reactiveLimits.volumetricDensity } },
            contrastCurve: { baseline: 0.88, profile: { range: 0.22, min: 0.4, max: 1.4, limit: this.parameterTuning.reactiveLimits.contrastCurve } },
            hueStart: { baseline: 170, profile: { range: -20, limit: this.parameterTuning.reactiveLimits.hueStart } },
            hueEnd: { baseline: 230, profile: { range: 36, limit: this.parameterTuning.reactiveLimits.hueEnd } },
            cameraTilt: { baseline: 0.04, profile: { range: 0.06, min: -1, max: 1, limit: this.parameterTuning.reactiveLimits.cameraTilt } },
            cameraSway: { baseline: 0.04, profile: { range: 0.06, min: -1, max: 1, limit: this.parameterTuning.reactiveLimits.cameraSway } }
        });
