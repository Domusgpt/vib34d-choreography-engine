...
export class FacetedSystem extends BaseSystem {
    ...
    update(deltaTime, parameters, audioData) {
        ...
        reactiveParams.saturation = clamp(
            (parameters.saturation || 0) + behavior.color.saturationPulse + behavior.color.spark * 0.25,
            0,
            1
        );

        reactiveParams.intensity = clamp(
            (parameters.intensity || 0.9) + behavior.color.spark * this.audioReactivity * 0.6 + behavior.light.glow * 0.25,
            0.1,
            3
        );
