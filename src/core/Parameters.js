...
export class ParameterManager {
    ...
    sanitizeProfile(profile = {}) {
        ...
        const limit = Number.isFinite(sanitized.limit) ? Math.abs(sanitized.limit) : Infinity;
        sanitized.limit = Math.max(0, limit);

        if (Number.isFinite(sanitized.min) && Number.isFinite(sanitized.max) && sanitized.min > sanitized.max) {
            const tmp = sanitized.min;
            sanitized.min = sanitized.max;
            sanitized.max = tmp;
        }

        sanitized.attack = Math.max(0, Number.isFinite(sanitized.attack) ? sanitized.attack : this.attack);
        sanitized.release = Math.max(0, Number.isFinite(sanitized.release) ? sanitized.release : this.release);

        sanitized.mode = sanitized.mode === 'multiply' ? 'multiply' : 'add';

        if (!Number.isFinite(sanitized.range)) {
            sanitized.range = 0;
        }
    ...
    setProfiles(profiles = {}) {
        Object.entries(profiles).forEach(([key, profile]) => {
            ...
            this.reactiveProfiles[key] = this.sanitizeProfile({
                ...existing,
                ...profile
            });
    ...
    setReactiveEnabled(map = {}) {
        ...
        // Keep baselines and offsets consistent with the latest gating state
        this.reapplyBaselines();
    }
    ...
    resolve(envelopes = {}, overrides = {}) {
        ...
        Object.entries(this.baseline).forEach(([key, baseValue]) => {
            ...
            const hitMin = Number.isFinite(profile.min) && value <= profile.min + 1e-6;
            const hitMax = Number.isFinite(profile.max) && value >= profile.max - 1e-6;
            const hitLimit = Number.isFinite(profile.limit) && Math.abs(newOffset) >= Math.abs(profile.limit) - 1e-6;
            const hitFloor = driver === 0 && driverRaw > 0;
