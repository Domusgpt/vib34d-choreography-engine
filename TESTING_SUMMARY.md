...
- `npm run visual:smoke` runs a headless Playwright dev server session, triggers the built-in visual test cycle, and captures a screenshot of the Mobile Smart stack for quick verification (run `npx playwright install chromium` once to grab the browser binary).
